...
\usepackage{tabularx}          % width-adaptive X columns
\usepackage{array}
...
\IEEEauthorblockA{\textit{Facultad de Ingenier\'ia, Instituci\'on Universitaria de Envigado}\\
Envigado, Colombia\\
ORCID: 0009-0001-9462-1136}
\and
\IEEEauthorblockN{Jhon Hander Mej\'ia-Mu\~noz}
\IEEEauthorblockA{\textit{Facultad de Ingenier\'ia, Instituci\'on Universitaria de Envigado}\\
Envigado, Colombia\\
ORCID: 0009-0008-7768-4072}
\and
\IEEEauthorblockN{Author 3 (placeholder)}
\IEEEauthorblockA{\textit{Facultad de Ingenier\'ia, Instituci\'on Universitaria de Envigado}\\
Envigado, Colombia}
\and
\IEEEauthorblockN{Author 4 (placeholder)}
\IEEEauthorblockA{\textit{Facultad de Ingenier\'ia, Instituci\'on Universitaria de Envigado}\\
Envigado, Colombia}
}
...
MaternaQA-es, while examining no retrieval, Hybrid retrieval, and
...
Semantic Similarity in both scenarios, whereas MedGemma~1.5 QLoRA without
retrieval achieved the highest observed mean Answer Relevancy. HyDE
...
heterogeneous and model-dependent. QLoRA improved all three no-retrieval
...
evidence of model or retrieval-strategy superiority. Some automatic metric
values were missing, underscoring the need for broader evaluation and
clinician review.
\end{abstract}
...
generated from 63 Spanish-language clinical documents. The released
document-level partitions contain 57 source PDFs: 52 in training, 2 in
validation, and 3 in test. The dataset is publicly available through
...
three planned levels: the no-retrieval control (\code{no_rag}), canonical
...
retrieval, and $N$ denotes the no-retrieval control. Analogous contrasts
...
test PDFs. This study processed 10 questions from the test partition; test
...
concatenated with the question. The separate \code{sft_closed_book}
representation contains only the question and is not a primary condition in
the retained comparison. The term base indicates whether an adapter is
...
result artifacts covered \code{no_rag}, Hybrid, and HyDE for all four
...
\midrule
C01 & Gemma 4 & Base & \code{no_rag} & Base weights; no retrieved context \\
...
C04 & Gemma 4 & QLoRA & \code{no_rag} & Grounded QLoRA adapter; no retrieved context \\
...
C07 & MedGemma 1.5 & Base & \code{no_rag} & Base weights; no retrieved context \\
...
C10 & MedGemma 1.5 & QLoRA & \code{no_rag} & Grounded QLoRA adapter; no retrieved context \\
...
quantized base-model weights with trainable low-rank adapter matrices,
reducing memory requirements while preserving the base model as a
separately identifiable initialization~\cite{dettmers2023qlora}.
Fig.~\ref{fig:qlora} contrasts full fine-tuning, which updates the base
weights, with LoRA, which freezes them and learns a low-rank update. In this
benchmark, QLoRA applies low-rank adaptation to quantized base weights; the
NF4 quantization and BF16 computation settings are specified in
Table~\ref{tab:qlora-config}. RAFT is a related method for domain-specific
RAG but is not an additional condition in this benchmark~\cite{zhang2024raft}.
...
\midrule
Base model & \code{google/gemma-4-E2B-it} & \code{google/medgemma-1.5-4b-it} \\
Training representation & \code{sft_grounded} & \code{sft_grounded} \\
...
Target modules & All linear (\code{all-linear}) & All linear (\code{all-linear}) \\
...
optimization settings. The exact mixed-precision mode of the retained
training runs could not be reliably recovered and is therefore not reported.
\end{tablenotes}
\end{threeparttable}
\end{table*}
...
retrieval strategy as a reproducible experimental factor. The no-retrieval
control, Hybrid retrieval, and independent HyDE differ in how evidence is
prepared before the answer model receives the question and context.
Hypothetical text is used only for query expansion; retrieved corpus chunks
form the evidence supplied to the answer model. Fig.~\ref{fig:rag-types}
...
\caption{RAG variants used in the benchmark: no retrieval, Hybrid retrieval
...
\code{batch_size=32}. The persistent local vector table is LanceDB.
...
Sparse retrieval is Okapi BM25 implemented with
\code{rank_bm25/BM25Okapi} over Unicode lowercase word tokens. BM25
supplies sparse term-weight scores and ranked chunk identifiers. Retrieval
outputs are recorded before answer generation so retrieval quality and
latency can be analyzed separately from response generation.

\subsubsection{Canonical Hybrid Retrieval}
Hybrid retrieval fuses dense and BM25 rankings. For a requested final depth
$k$, the candidate depth is:
\begin{equation}
\label{eq:candidate-depth}
k_c = \min(\lvert C \rvert,\, \max(4k, 50)).
\end{equation}
Each retriever contributes ranked candidates, and the union is fused by
deterministic reciprocal rank fusion with \code{rank_constant=60}%
~\cite{cormack2009reciprocal}. Ties are broken by stable chunk ID. The
final top-$k$ chunks are supplied to the answer model.
...
conditions; retrieved corpus chunks, rather than the potentially
hallucinatory hypothetical text itself, were supplied as evidence to the
answer model~\cite{ji2023survey}. The request-side output limit must be
reported from the run configuration because the consolidated workbook
records token usage but not that maximum setting.

The cache is separated by dataset mode, provider, and generator model; row
reuse checks \code{qa_id} and a question hash. Dense retrieval embeds the
hypothetical text with BGE-M3 and searches the corpus by exact cosine
similarity. Retrieved chunks, not the hypothetical text, are supplied as
evidence to the answer model.

\subsubsection{No-Retrieval Control}
The no-retrieval control (\code{no_rag}) returns no retrieved contexts and
...
answer prompt. The \code{no_rag} condition receives the same question
...
Similarity. Context Precision evaluates whether relevant chunks are ranked
before irrelevant ones. Context Recall estimates how much
reference-supporting information is retrieved. Faithfulness measures
whether claims are supported by supplied context. Answer Relevancy measures
whether the response addresses the question. Answer Correctness compares
the response with the reference answer, and Semantic Similarity measures
...
hardware-independent throughput. For \code{no_rag}, Context Precision,
Context Recall, and Faithfulness are missing by design rather than zero.
\begin{equation}
\label{eq:tps}
...
scenario; missing evaluator values are not imputed. Given the limited
number of evaluated questions per scenario, all comparisons are interpreted
as exploratory descriptive patterns and are not intended to establish
definitive model or retrieval-strategy superiority. QLoRA is compared with
...
conditions in each scenario. Project scope, processing time, and variable
metric coverage motivated a descriptive rather than confirmatory analysis.
...
\code{sample10}---evaluated across all 12 conditions. MaternaQA-es retained
grounded source context with the question, whereas \code{sample10}
contained only the question before retrieved context was added. Because
scenario differences may reflect both question composition and input
grounding, direct cross-scenario metric comparisons are not used to infer
effects of fine-tuning or retrieval. Principal comparisons are made within
each scenario.
...
conditions, and all three inference strategies. \code{no_rag}, Hybrid, and
...
without imputation. Accordingly, the results reported below should be
interpreted as descriptive comparisons across these two 10-question
evaluation subsets.
...
non-applicability for \code{no_rag}. All conditions yielded 10 responses;
valid $n$ was 5--10 for response metrics and 6--10 for retrieval metrics
because some evaluator outputs were unavailable. No missing value was
imputed; bold values are panel maxima.
\end{tablenotes}
\end{threeparttable}
\end{table*}
...
QLoRA without retrieval obtained the highest observed mean Answer Relevancy
(0.811). Under no retrieval, QLoRA increased Gemma~4 Answer Correctness by
...
Relative to \code{no_rag}, Hybrid and HyDE produced their largest
...
Semantic Similarity (0.736), whereas MedGemma~1.5 QLoRA without retrieval
obtained the highest observed mean Answer Relevancy (0.881). Under no
retrieval, QLoRA improved all three response metrics for both model
families.

Relative to \code{no_rag}, Hybrid increased Answer Correctness for all
...
Relative to \code{no_rag}, HyDE increased Answer Correctness for Gemma~4
...
Faithfulness, Context Precision, and Context Recall were not calculated for
\code{no_rag} because those conditions did not supply retrieved evidence.
Their absence represents non-applicability rather than zero performance.

\subsection{Run and Metric Coverage}
All 24 scenario-condition runs generated responses for their 10 shared
questions; therefore, there were no condition-level execution failures in
the retained matrix. The workbook nevertheless contained unavailable
automatic scores in 20 of the 240 selected condition-item records (26
missing applicable metric cells in MaternaQA-es and 31 in the obstetric
challenge set). These values were left missing rather than imputed, and
Table~\ref{tab:results} reports means over the valid scores available for
each metric. Context-dependent metrics remained not applicable to
\code{no_rag} by design.
...
no-retrieval conditions, QLoRA improved Answer Correctness, Answer
...
without retrieval in both scenarios, suggesting that medical initialization
...
answers. For the MaternaQA-es subset, however, the supporting source
material was available in the retrieval corpus. These results therefore
characterize retrieval of available evidence rather than strict
generalization to completely unseen source documents.

The study evaluated 10 MaternaQA-es questions and 10 obstetric-challenge
questions across all 12 conditions. MaternaQA-es retained grounded source
context, whereas \code{sample10} began from question-only records. This
asymmetry may influence response quality and retrieval effects, so
conclusions emphasize within-scenario patterns.

% =====================================================================
\section{Conclusions}
% =====================================================================
In this exploratory 20-question benchmark, grounded QLoRA improved the
no-retrieval response metrics of Gemma~4 across both scenarios, while its
...
Hybrid retrieval. The retrieval findings should be interpreted as
evidence-available retrieval performance rather than strict generalization
to completely unseen source documents. The evidence therefore supports a
nuanced conclusion: observed benefits varied by model family, metric, and
scenario. Because the two evaluation scenarios differ in question
composition and initial grounding, absolute metric differences between them
should not be interpreted as direct effects of fine-tuning or retrieval.
Given the limited evaluation of 10 questions per scenario, these findings
should be considered preliminary, and broader question coverage, complete
evaluation, and clinician assessment remain necessary before drawing
deployment-oriented conclusions.

% =====================================================================
\section*{Declaration of Generative AI and AI-Assisted Technologies}
% =====================================================================
The authors used ChatGPT~\cite{openai2026chatgpt} only to support language
editing, code and reference review, figure preparation, and document
formatting. It was not used to generate, modify, or evaluate experimental
data or to make clinical judgments. All AI-assisted outputs were reviewed
by the authors, who take responsibility for the final content.
