...
Retained pages are grouped into clinically coherent text units that balance answerability with generation constraints. The construction script applies configurable boundaries with a default minimum of 500 tokens, maximum of 1,200 tokens, and a 100-token overlap between adjacent chunks computed with the model tokenizer. Candidate chunks are filtered by an accepted-token threshold and a clinical-score heuristic estimating the proportion of domain-relevant terminology. Each chunk receives a stable identifier and is annotated with source PDF, page range, document type, section type, content role, topic tags, token estimate, and clinical score.
